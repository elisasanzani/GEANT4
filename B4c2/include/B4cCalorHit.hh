...
class B4cCalorHit : public G4VHit
{
  public:
    ...
    void Add(G4double de, G4double dl, G4double xc, G4double yc);

    // get methods
    G4double GetEdep() const;
    G4double GetTrackLength() const;
    G4double GetCentroidX() const;
    G4double GetCentroidY() const;

      
  private:
    G4double fEdep;        ///< Energy deposit in the sensitive volume
    G4double fTrackLength; ///< Track length in the  sensitive volume
    G4double fPosEnX;    ///<Product of position in X axis per energy dep
    G4double fPosEnY;    ///<Product of position in Y axis per energy dep
    //G4ThreeVector fCentroid;     ///< Position of the centoid in the sensitive volume
};
...
inline void B4cCalorHit::Add(G4double de, G4double dl, G4double xc, G4double yc) {
  fEdep += de; 
  fTrackLength += dl;
  fPosEnX += xc * de;
  fPosEnY += yc * de;
...
inline G4double B4cCalorHit::GetCentroidX() const {
    return fPosEnX/fEdep;
}

inline G4double B4cCalorHit::GetCentroidY() const {
    return fPosEnY/fEdep;
}
