...
{  
  ...
  // step hit position
  G4ThreeVector stepHit = step->GetPreStepPoint()->GetPosition();
    G4double XstepHit = stepHit.x();
    G4double YstepHit = stepHit.y();
  ...
  // Add values
  hit->Add(edep, stepLength, XstepHit, YstepHit);
  hitTotal->Add(edep, stepLength, XstepHit, YstepHit); 
